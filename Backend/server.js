...
app.post("/auth", async (req, res) => {
  const { action, username, email, password, dob, number, refreshToken } =
    req.body;
...
// GET: Get user by emailID
app.get("/selectuserbyemail", async (req, res) => {
  const { email } = req.query; // Assume the user ID is passed as a query param
  try {
    // Fetch user by ID from the database
    const result = await db.getUserByEmail(email); // `id` represents the userId
    if (!result) {
      return res.status(404).json({ message: "User not found" });
    }
